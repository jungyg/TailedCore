...
class FeatureEmbedderTransformer(torch.nn.Module):
    ...
    def __init__(
        ...
        super(FeatureEmbedderTransformer, self).__init__()
    ...
    def forward(
        ...
        if return_embeddings:
            x = self.transformer.forward_features(images)
            embeddings = self.transformer.forward_head(x, pre_logits=True)
